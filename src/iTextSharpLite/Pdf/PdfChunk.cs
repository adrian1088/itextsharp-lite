...
namespace iTextSharp.Pdf
{
    ...
    public class PdfChunk
    {
        ...
        internal PdfChunk(Chunk chunk, PdfAction action)
        {
            ...
            Font f = chunk.Font;
            float size = f.Size;
            if (size == iTextSharp.Font.UNDEFINED)
                size = 12;
            baseFont = f.BaseFont;
            BaseFont bf = f.BaseFont;
            int style = f.Style;
            if (style == iTextSharp.Font.UNDEFINED)
            {
                style = iTextSharp.Font.NORMAL;
            }
            if (baseFont == null)
            {
